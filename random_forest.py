...
# Fill NaNs before calculating demand_score
df['Quantity'] = df['Quantity'].fillna(0)
df['Sell Price'] = df['Sell Price'].fillna(df['Sell Price'].mean())
df['Rating'] = df['Rating'].fillna(df['Rating'].mean())

# Create demand score and demand level
df['demand_score'] = (
    df['Quantity'] + 
...
clf_numeric = RandomForestClassifier(n_estimators=100, random_state=42)
...
plt.title('Random Forest Decision Regions (PCA-reduced, 10k Sample)')
